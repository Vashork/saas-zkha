...
Verifies that every mutation route creates an audit log entry via log_admin_action.
"""
...
@pytest.mark.asyncio
async def test_delete_contractor_creates_audit_log(audit_db):
    session, factory, engine, admin = audit_db
...
async def test_add_transaction_creates_audit_log(audit_db):
    ...
    async with factory() as session2:
        ...
        session2.add(payment)
        await session2.commit()

    request = _request("/payments/p-add-tx/transactions/add", user=admin)

    response = await payments.add_payment_transaction(
        "p-add-tx", request, db=session,
...
async def test_delete_transaction_creates_audit_log(audit_db):
    ...
    request = _request("/payments/transactions/tx-edit-test/edit", user=admin)

    response = await payments.edit_payment_transaction(
        "tx-edit-test", request, db=session,
