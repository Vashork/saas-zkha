...
- [ ] Переписать миграции в `init_db.py` без raw `cursor()`, через `conn.execute(text(...))`.
- [ ] Проверить, что `Dockerfile.web` по-прежнему успешно выполняет `python init_db.py` перед запуском web.

### Этап 2 — `/backups` UI

- [ ] Добавить кнопку скачивания backup-файла в `backups.html`.
- [ ] Исправить текст страницы: архивы сохраняются в `./backups` на хосте через Docker volume.
- [ ] Проверить, что download route остаётся admin-only.

### Этап 3 — навигация

- [ ] Добавить ссылку `💾 Бекапы` для admin во все navbar-шаблоны.
...
- [ ] Заменить `float` на `Decimal` при работе с `fixed_amount` подрядчиков.
- [ ] Разрешить ручное создание variable payment без суммы.
- [ ] Привести CSV export истории к тем же фильтрам, что страница истории: год, месяц, подрядчик, статус.
