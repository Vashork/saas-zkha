# LOCAL AI TODO

Файл ведётся как актуальный хвост задач для локального ИИ.
Выполненные пункты удаляются из раздела актуальных задач.

## Актуальные задачи

### 1. Timezone в настройках

Добавить в настройки поле timezone.
Сохранять значение в `settings.notification_timezone`.
Использовать это значение для времени на странице бекапов.

### 2. Порядок вкладок

Нужный порядок:
Дашборд → Платежи → Подрядчики → Аналитика → История → Бекапы → Настройки.

Проверить все страницы с верхним меню и привести их к этому порядку.

### 3. Select и блок бекапов

Создан файл `app/web/static/css/local-ui-tweaks.css`.
Нужно подключить его в базовом шаблоне.
Он содержит:
- правый отступ для выпадающих списков;
- увеличенный отступ в блоке настройки бекапов;
- увеличенный line-height для карточек `Локально` и `Mounted share`.

## Выполнено

- Создан `docs/LOCAL_AI_TODO.md`.
- Создан `docs/LOCALAL.md`.
- Создан `app/web/static/css/local-ui-tweaks.css`.
- На странице analytics порядок меню и отступы select уже поправлены.
