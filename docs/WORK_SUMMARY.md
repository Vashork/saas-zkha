...
- После `docker compose restart` старая авторизация должна слететь и открыть login.
...
7. Проверить Telegram тегами без файла.
8. Проверить Telegram тегами в подписи к файлу/фото.
9. Проверить Telegram со старым периодом.
10. Проверить Telegram без тегов: файл/фото → подрядчик → сумма → период → подтверждение.
11. Проверить `/cancel`.
